...
The open card is as wide as its longest task, between 300 and 440 points; a longer title is cut with an ellipsis and shows in full as a tooltip. It grows down to fit every task, up to 60 percent of the screen height; past that the list scrolls inside it.

Crossing a task off plays a short pen scratch. See Sounds below.

## Sounds

Crossing a task off plays "Pen", a short scratch generated by `scripts/make-sound.swift`, one per line as the ink draws. Right click the notch and open "Sound" to pick another one or "Off". The app also stays quiet when "Play user interface sound effects" is off in System Settings.

To add your own, drop audio files (mp3, m4a, wav, aiff, caf) into `~/.config/mainthing/sounds/`. They show up in the menu by file name, dashes as spaces, title cased: `texture-scratch.mp3` is "Texture Scratch". Picking one plays it once. A custom sound plays once per cross off, to its end; a new cross off stops it. Every sound is matched to the pen's loudness from its own samples, so a file that peaks at 0 dB does not blast. If the chosen file goes away, the app falls back to Pen.

Only add sounds you have the rights to. That folder is yours, outside the repo; `sounds/`, `*.mp3` and `*.m4a` are in `.gitignore` so none of them get committed by accident.
