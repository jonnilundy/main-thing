...
func runPathChecks() {
    ...
    check("the env file wins for a test id", AppPaths.tasksFile(environment: ["MAINTHING_TASKS_FILE": "/tmp/t.json"], bundleID: test, applicationSupport: support).path == "/tmp/t.json")
    check("the env file wins for the release id", AppPaths.tasksFile(environment: ["MAINTHING_TASKS_FILE": "/tmp/t.json"], bundleID: MainThingBundleID, applicationSupport: support).path == "/tmp/t.json")
    check("an empty env file is ignored", AppPaths.tasksFile(environment: ["MAINTHING_TASKS_FILE": ""], bundleID: test, applicationSupport: support) == keyed)
    ...
    check("release config is ~/.config/mainthing", config.path == "/Users/me/.config/mainthing")
    check("test config is keyed", AppPaths.configDirectory(environment: none, bundleID: test, home: home).path == "/Users/me/.config/mainthing-com.jonnilundy.mainthing.test")
    check("XDG_CONFIG_HOME is kept for a test id", AppPaths.configDirectory(environment: ["XDG_CONFIG_HOME": "/x"], bundleID: test, home: home).path == "/x/mainthing-com.jonnilundy.mainthing.test")
    check("XDG_CONFIG_HOME for the release id", AppPaths.configDirectory(environment: ["XDG_CONFIG_HOME": "/x"], bundleID: nil, home: home).path == "/x/mainthing")
    check("the env config dir wins", AppPaths.configDirectory(environment: ["MAINTHING_CONFIG_DIR": "/c"], bundleID: test, home: home).path == "/c")
